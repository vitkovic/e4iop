    ...
    <div v-if="cmsPage" class="container">
      <div class="row">
        <div class="col-xs-12 mb-5">
          ...
          <h1 v-text="$t('global.menu.about')"></h1>
        </div>
        <div class="col-md-12">
          <div v-html="cmsPage.content" class="contentText p-2" style="white-space: preserve;"></div>
        </div>
      </div>
      ...
      <section v-if="cmsPageImages.length > 0" class="section-gallery mb-4">
        ...
        <div class="wrapper">
          <div class="carousel" ref="carousel" v-if="companyImagesArray">
            <img
              v-for="(image, index) in cmsPageImages"
              :key="image.id"
              @click="onPreviewImage(index)"
              :src="retrieveFile(image)"
              alt="img"
              @load="onImageLoad"
            />
          </div>
