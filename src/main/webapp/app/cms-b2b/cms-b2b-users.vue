        ...
        <div class="custom-table-responsive" v-if="users && users.length > 0">
            <table class="table table-striped">
                ...
                <tr>
                    ...
                    <th v-on:click="changeOrder('portalUser.company.name')"><span v-text="$t('riportalApp.company.detail.title')">Status</span> <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'portalUser.company.name'"></jhi-sort-indicator></th>

                    <!-- <th><span v-text="$t('riportalApp.company.detail.title')">Company</span></th> -->
                ...
                <tr v-for="user in users"
                    ...
                    <td>{{ user.email }}</td>
                    <td>
                        <div v-if="user.companyName && user.companyId">
                            <router-link :to="{name: 'CompanyView', params: {companyId: user.companyId}}" class="text-body">{{user.companyName}}</router-link>
                        </div>
                    </td>
...
<script lang="ts" src="./cms-b2b-users.component.ts"></script>
