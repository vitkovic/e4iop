<template>
  <div>
    <router-link
      :to="{ name: 'CmsQuestionCreate' }"
      tag="button"
      id="jh-create-entity"
      class="btn btn-primary float-right jh-create-entity create-cms-question"
    >
    ...
    <div class="custom-table-responsive" v-if="cmsQuestions && cmsQuestions.length > 0">
      <table class="table table-striped">
        <thead>
          <tr>
            ...
            <th v-on:click="changeOrder('question')">
            ...
            <th v-on:click="changeOrder('answer')">
              <span v-text="$t('riportalApp.cmsQuestion.answer')">Answer</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'answer'"></jhi-sort-indicator>
            </th>
        ...
        <tbody>
          <tr v-for="cmsQuestion in cmsQuestions" :key="cmsQuestion.id">
            <td>
              <router-link :to="{ name: 'CmsQuestionView', params: { cmsQuestionId: cmsQuestion.id } }" class="text-body">{{ cmsQuestion.id }}</router-link>
            </td>
            ...
            <td class="text-right">
              <div class="btn-group">
                <router-link
                  :to="{ name: 'CmsQuestionView', params: { cmsQuestionId: cmsQuestion.id } }"
                  tag="button"
                ...
                <router-link
                  :to="{ name: 'CmsQuestionEdit', params: { cmsQuestionId: cmsQuestion.id } }"
                  tag="button"
                ...
                <b-button v-on:click="prepareRemove(cmsQuestion)" variant="danger" class="btn btn-sm" v-b-modal.removeEntity>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="riportalApp.cmsQuestion.delete.question" v-text="$t('entity.delete.title')">Confirm delete operation</span></span
      >
      <div class="modal-body">
        <p id="jhi-delete-cmsQuestion-heading" v-text="$t('riportalApp.cmsQuestion.delete.question', { id: removeId })">
          Are you sure you want to delete this Cms Question?
        </p>
      </div>
...
<script lang="ts" src="./cms-question.component.ts"></script>
