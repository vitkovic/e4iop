<template>
  <div>
    <h2 id="page-heading" class="mt-5">
      ...
      <router-link
        :to="{ name: 'CMSNewsCreate' }"
        tag="button"
        id="jh-create-entity"
        class="btn btn-primary float-right jh-create-entity create-cms-news"
      >
    ...
    <div class="custom-table-responsive" v-if="cmsNews && cmsNews.length > 0">
          ...
          <tr v-for="news in cmsNews" :key="news.id">
            ...
            <td class="text-right">
              <div class="btn-group">
                ...
                <b-button @click="prepareRemove(news)" variant="danger" class="btn btn-sm" v-b-modal.removeEntity>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      ...
      <div slot="modal-footer">
        <button
          ...
          id="jhi-confirm-delete-cmsNews"
          v-text="$t('entity.action.delete')"
          @click="removeCmsNews()"
        >
          Delete
        </button>
        <button type="button" class="btn btn-danger" v-text="$t('entity.action.cancel')" @click="closeDialog()">Cancel</button>
      </div>
    </b-modal>
...
<script lang="ts" src="./cms-news.component.ts"></script>
